...
df.TARGET <- res.TARGET$DA.results$DA.peaks$FavorablePromyelocytes_InterPromyelocytes 
...
res.TCGA <- cinaR(count.TCGA[,c(TRUE,idx.TCGA)], contrasts = temp.TCGA$ELN.Pro2[idx.TCGA], experiment.type = "RNA-Seq", comparison.scheme = "OVO", DA.fdr.threshold = 1, DA.lfc.threshold = 0)
...
```
```{r ATRA related genes}
ATRA.genes <- read.csv("data/ATRA_effected_genes.csv", sep = ";")[,c(1,2)]
...
count.BEAT <- filterConsensus(count.BEAT)
count.BEAT <- edgeR::cpm(count.BEAT)

ann.col.BEAT <- temp.BEAT[,c("ELN.Pro", "cluster.refined"), drop = F]
ann.row.BEAT <- ATRA.genes[,"Status", drop = F]
rownames(ann.row.BEAT) <- ATRA.genes$Genes


pdf("output/BEAT_ATRA_induced_genes_heatmap.pdf", width = 10, height = 10)
pheatmap(count.BEAT[ATRA.genes[(ATRA.genes$Genes) %in% rownames(count.BEAT), "Genes"] , ], scale = "row", border_color = NA, 
         cluster_row = F, annotation_row = ann.row.BEAT,  annotation_col =  ann.col.BEAT,
         color = pheatmap.colors, breaks = breaksList, show_colnames = F)
dev.off()
