...
DimPlot(vg.all, group.by = "CellType", label = F, shuffle = T,pt.size = 0.6, cols = sample(DiscretePalette(22)))
...
FeaturePlot(vg.all, features) & theme(legend.key.height= unit(0.3, 'cm'), legend.key.width= unit(0.3, 'cm'))
plot_density(vg.all, features) & coord_fixed(ratio = 1) & theme(legend.key.height= unit(0.3, 'cm'), legend.key.width= unit(0.3, 'cm'), legend.title = element_text(size=6), legend.text = element_text(size=5)) & FontSize(
  x.text = 7,
  y.text = 7,
  x.title = 7,
  y.title = 7,
...
VlnPlot(vg.all, c(features, "GAPDH", "HSP90", "ACTB", "HMBS", "PBGD") , 
        group.by = "CellType", stack = T, sort = F, flip = TRUE) + 
