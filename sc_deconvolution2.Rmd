...
meta.TCGA <- meta.TCGA %>% mutate(Age = age_at_diagnosis/365, 
                                  ...
                                  ELN = RISK_MOLECULAR, 
                                  Status = ifelse(temp.TCGA$event == "Alive", 0,1),
                                  Patient = patient_ID,
                                  Sample = sample_ID,
                                  Study = "TCGA") 
...
ann.row.BEAT <- as.data.frame(cbind.data.frame(pdiagnoses.BEAT, Blast = meta.BEAT$blasts, Stemness = meta.BEAT$stemness, FAB = meta.BEAT$FAB, ELN = meta.BEAT$ELN))
...
meta.LEUCE <- meta.LEUCE %>% mutate(Age = NA, 
                                  Sex = NA, 
                                  Recurrence = NA,
...
df.blasts <- read.csv("data/Studies_Latest/Blast_percentages_all.csv", sep = ";", row.names = 1)

mt <- data.frame(meta.TARGET, cluster = clusters.TARGET)
mt <- merge(mt, df.blasts, by.x = "Sample", by.y = "sample_ID", all.x = T)

# one way anova 
res.aov <- aov(blasts_measured ~ cluster, data=mt)
summary(res.aov)
...
ggplot(mt, aes(cluster, blasts_measured, fill = cluster)) + geom_point() + geom_boxplot() + coord_flip() + scale_fill_manual(values = cell.type.colors) + xlab("") + ylab("Reported Blast %")+theme.gg + 
  geom_hline(yintercept = mean(mt$blasts_measured, na.rm = T))


ggplot(mt, aes(cluster, Age, fill = cluster)) + geom_point() + geom_boxplot() + coord_flip() + scale_fill_manual(values = cell.type.colors) + xlab("") + ylab("Age")+theme.gg


mb <- data.frame(meta.BEAT, cluster = clusters.BEAT)

ggplot(mb, aes(cluster, blasts, fill = cluster)) + geom_point() + geom_boxplot() + coord_flip() + scale_fill_manual(values = cell.type.colors) + xlab("") + ylab("Reported Blast %")+theme.gg  +
  geom_hline(yintercept = mean(mb$blasts, na.rm = T))


ggplot(mb, aes(cluster, Age, fill = cluster)) + geom_point() + geom_boxplot() + coord_flip() + scale_fill_manual(values = cell.type.colors) + xlab("") + ylab("Age")+theme.gg
```
...
common.features <- c("Sample", "Patient","Study","Age", "Sex","Origin","Primary Diagnosis", 
...
plot.df <- data.frame(UMAP = umap$layout, TSNE = tsne$Y, meta.train, check.names = F)
plot.df$`Primary Diagnosis` <- gsub("Acute myeloid leukemia", "", plot.df$`Primary Diagnosis`)

pdf("output/UMAP_ALL_patients.pdf")

plot.celltype <- ggplot(plot.df, aes(UMAP.1, UMAP.2, color = cluster)) +
  geom_point(size = 2) +
  theme_light() + 
  coord_fixed(ratio = 1) + 
...
plot.sex <- ggplot(plot.df, aes(UMAP.1, UMAP.2, color = Sex)) +
  geom_point(size = 2) +
  theme_light() + 
  coord_fixed(ratio = 1) + 
...
plot.stemness <- ggplot(plot.df, aes(UMAP.1, UMAP.2, color = Stemness)) +
  geom_point(size = 2) +
  theme_light() + 
  coord_fixed(ratio = 1) + 
...
plot.APS <- ggplot(plot.df, aes(UMAP.1, UMAP.2, color = APS)) +
  geom_point(size = 2) +
  theme_light() + 
  coord_fixed(ratio = 1) + 
...
plot.fab<- ggplot(plot.df[sample(1:nrow(plot.df)),] , aes(UMAP.1, UMAP.2, color = FAB)) +
  geom_point(size = 2) +
  theme_light() + 
  coord_fixed(ratio = 1) + scale_color_manual(values = ann.colors$FAB)
plot.fab

plot.eln<- ggplot(plot.df[sample(1:nrow(plot.df)),] , aes(UMAP.1, UMAP.2, color = ELN)) +
  geom_point(size = 2) +
  theme_light() + 
...
lapply(c("NPM1", "FLT3", "IDH2", "WT1", "TET2", "JAK2", "MYC", "NRAS"), function(x){
  ...
  geom_point(size=1, data=select(plot.df, -.data[[x]]), color = "gray") +
...
```{r NPM1}
df.npm1 <- meta.train %>% filter(NPM1 == 1)

model.npm1.sex <- survfit(Surv(overal_survival,  Status) ~ Sex, data=df.npm1)
model.npm1.sex
ggsurvplot(model.npm1.sex, pval = T)
