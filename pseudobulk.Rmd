...
predictions <- read.delim('../../Downloads/azimuth_pred.tsv', row.names = 1)
...
pdf("output/UMAP_Integrated_cell_type_labeled.pdf", width = 14, height = 6)
DimPlot(immune.combined, repel = T, label.size = 4, shuffle = T, cols = DiscretePalette(29)) + theme.gg
dev.off()

pdf("output/UMAP_Integrated_cell_type_labeled_azimuth.pdf", width = 14, height = 6)
DimPlot(immune.combined, group.by = "predicted.celltype.l2", repel = T, label = T,label.box = T, label.size = 2.5, shuffle = T, cols = DiscretePalette(40)) + theme.gg
dev.off()


pdf("output/UMAP_Integrated_sample_labeled.pdf", width = 8, height = 6)
DimPlot(immune.combined, group.by = "Sample", shuffle = T) + theme.gg + ggtitle("")
dev.off()


pdf("output/UMAP_Integrated_study_labeled.pdf", width = 8, height = 6)
DimPlot(immune.combined, group.by = "study", shuffle = T) + theme.gg + ggtitle("")
dev.off()
...
p1 <- DimPlot(hcabm40k, group.by = "predicted.id", label = T, repel = T, label.size = 2.5)  + 
  theme.gg + ggtitle ("") & NoLegend() 
...
```{r HCA subset sample based deconvolution}

load("cache/cell_type_colors.RDS")

excluded.cell.types <- cell.type.order[!cell.type.order %in% names(cell.type.colors)]

cell.type.colors.2<- as.character(paletteer_d("ggsci::nrc_npg"))
names(cell.type.colors.2) <- excluded.cell.types
...
original.proportions <- meta %>% group_by(orig.ident) %>% 
  add_count(name = "total.cell.per.sample") %>% 
  group_by(orig.ident, predicted.id, total.cell.per.sample) %>% 
  summarise(total.cell.per.ct = n()) %>% 
  mutate(Fraction = total.cell.per.ct/total.cell.per.sample)


hca.samples.pb <- as.data.frame(AggregateExpression(hcabm40k, assays = "RNA", slot = "count", group.by = "orig.ident"))
T.eset <- ExpressionSet(assayData = as.matrix(hca.samples.pb))

sim.results <- music_prop(bulk.eset = T.eset, sc.eset = C.eset, clusters = 'label',
                                            markers = NULL, normalize = FALSE, samples = 'Sample', 
                                            verbose = F)

sim.results.df <- sim.results$Est.prop.weighted %>% reshape2::melt()

sim.results.df$Var1 <- gsub("RNA.", "", sim.results.df$Var1)



original.proportions$predicted.id <- factor(original.proportions$predicted.id,
                                            levels = cell.type.order[ cell.type.order %in% original.proportions$predicted.id])

p1 <- 
ggplot(original.proportions, aes(orig.ident, Fraction, fill = predicted.id )) + geom_bar(stat = "identity") + 
    scale_fill_manual(values = c(cell.type.colors[1:20], cell.type.colors.2[1:9])) + theme.gg + labs(fill = "Cell Types") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + xlab("Patients") + ylab("Cell Type Fraction") + 
    theme(strip.background = element_blank(), strip.text = element_blank()) + coord_flip()


sim.results.df$Var2 <- factor(sim.results.df$Var2,
                                            levels = cell.type.order[ cell.type.order %in% sim.results.df$Var2])

p2 <- 
ggplot(sim.results.df, aes(Var1, value, fill = Var2)) + geom_bar(stat = "identity") + 
    scale_fill_manual(values = cell.type.colors[1:20]) + theme.gg + labs(fill = "Cell Types") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + xlab("Patients") + ylab("Cell Type Fraction") + 
    theme(strip.background = element_blank(), strip.text = element_blank()) + coord_flip()

pdf("output/HCA_subset_deconvolution_per_sample.pdf", width = 8, height = 6)
...
```{r Predicting the Stemness Score}
# stemness score https://sci-hub.se/https://www.nature.com/articles/nature20598
stem_genes <- c("ENSG00000088305", "ENSG00000130584", "ENSG00000205978", "ENSG00000128805", "ENSG00000104341", "ENSG00000138722", "ENSG00000113657", "ENSG00000277988", "ENSG00000105810", "ENSG00000088882", "ENSG00000120833", "ENSG00000095932", "ENSG00000134531", "ENSG00000166681", "ENSG00000174059", "ENSG00000196139", "ENSG00000205336")
...
stemness.score <- function(counts){
  apply(log2(edgeR::cpm(counts, log = F) + 1)[stem_genes,], 
        2, 
        function(x){sum(x * stem_coefficients)})
}
```


```{r assigning colors to cell types}

cell.type.colors <- c(as.vector(paletteer::paletteer_d("ggthemes::Tableau_20")), "#7d1a1a", "#595959")

names(cell.type.colors) <- (c(colnames(deconv.LUMC), "Mixed Profiles", "Others"))
...
# calculate 'stemness score'
stemness.score.TCGA <- stemness.score(count.TCGA)
...
meta.TCGA$stemness <- stemness.score.TCGA

pdiagnoses.TCGA <- stats::model.matrix(~ 0 + meta.TCGA$primary_diagnosis)
colnames(pdiagnoses.TCGA) <- sub(pattern = "meta.TCGA\\$primary_diagnosis", replacement = "", colnames(pdiagnoses.TCGA))
...
# with mutations
# ann.row.TCGA <- cbind(pdiagnoses.TCGA, mut.TCGA[,mutations], Blast = meta.TCGA[,"BM_blasts"], Stemness = meta.TCGA[,"stemness"])
ann.row.TCGA <- as.data.frame(cbind(pdiagnoses.TCGA, Blast = meta.TCGA[,"BM_blasts"], Stemness = meta.TCGA[,"stemness"]))
...
pheatmap(deconv.TCGA [new.order.TCGA, intersect(cell.type.order, colnames(deconv.TCGA))], cluster_cols = F, cluster_rows = F, annotation_row = cbind(ann.row.TCGA[new.order.TCGA,], cluster = clusters.TCGA[new.order.TCGA]),show_rownames = F,
...
temp.TCGA <- cbind(meta.TCGA, cluster = cutree(hc.TCGA, k = 3))
temp.TCGA$status <- ifelse(temp.TCGA$event == "Alive", 0,1)

library(survminer)


temp.TCGA$cluster <- clusters.TCGA
...
```{r TCGA Stemness for each group}

pdf("output/TCGA_Stemness_vs_Refined_Clusters.pdf", width = 5, height = 5)
ggplot(temp.TCGA, aes(cluster.refined, stemness, color = cluster.refined)) + geom_boxplot() + geom_jitter(width = 0.1) + 
  theme.gg + ylab("Stemness Score") + xlab("") + ggtitle("TCGA") + 
  scale_color_manual(values = paletteer_d("ggsci::nrc_npg")) + 
  labs(color = "Redefined Clusters") + theme(axis.text.x=element_text(angle=45,hjust=1)) 
...
ann.row.LUMC <- cbind(mut.LUMC[,c(mutations,"FLT3.ITD", "FLT3.TKD")], Blast = meta.LUMC[,"blast_percentage"], `Primary Diagnosis` = meta.LUMC[, "primary_diagnosis"] )
...
pdf("output/Music_LUMC_deconvolution_via_integrated_pseudobulk_simple_clustering.pdf", width = 12, height = 12)
pheatmap(deconv.LUMC [new.order.LUMC, intersect(cell.type.order, colnames(deconv.LUMC))], cluster_cols = F, cluster_rows = F, annotation_row = cbind(ann.row.LUMC[new.order.LUMC,c(1,2,3,6,7)], cluster = clusters.LUMC[new.order.LUMC]),
         color = pheatmap.colors, breaks = breaksList, border_color = NA, fontsize_row = 5)
dev.off()
```
...
pheatmap(deconv.BEAT [new.order.BEAT, intersect(cell.type.order, colnames(deconv.BEAT))], cluster_cols = F, cluster_rows = F, annotation_row = cbind(ann.row.BEAT[new.order.BEAT,], cluster = clusters.BEAT[new.order.BEAT]),
         color = pheatmap.colors, breaks = breaksList, border_color = NA, fontsize_row = 5)
dev.off()
```
...
temp.BEAT$cluster <- clusters.BEAT

temp.BEAT <- temp.BEAT %>%  dplyr::add_count(cluster) 

temp.BEAT$cluster.refined <- ifelse(temp.BEAT$n > 20, temp.BEAT$cluster, "Others")
...
pdf("output/Music_TARGET_deconvolution_via_integrated_pseudobulk_simple_clustering.pdf", width = 12, height = 12)
pheatmap(deconv.TARGET [new.order.TARGET, intersect(cell.type.order, colnames(deconv.TARGET))], cluster_cols = F, cluster_rows = F, annotation_row = cbind(ann.row.TARGET[new.order.TARGET,c(1,2,4,3)], cluster = clusters.TARGET[new.order.TARGET]),
         color = pheatmap.colors, breaks = breaksList, border_color = NA, fontsize_row = 5)
dev.off()
```
...
```{r}
# SURVIVAL ALL
survival.idx <- c("overal_survival", "cluster", "status")

survival.all <- rbind(temp.TCGA[,survival.idx],
                      temp.BEAT[,survival.idx],
                      temp.TARGET[,survival.idx])

survival.all <- survival.all %>% add_count(cluster)

survival.all$cluster.refined <- ifelse(survival.all$n > 30, survival.all$cluster, "Others")

km_trt_fit <- survfit(Surv(overal_survival, status) ~ cluster.refined, survival.all)
pdf("output/All_Kaplan_Meier_simple_clustering.pdf")
p <- ggsurvplot(km_trt_fit, 
           pval = T,
           # surv.median.line = "hv", # Add medians survival
...
```{r fit(mut ~ cell props)}
library(randomForest)

common.mutations.filtered <- 
sapply(common.mutations, function(x){sd(mut.TCGA[,x])}) > 0 &
sapply(common.mutations, function(x){sd(mut.LUMC[,x])}) > 0 &
sapply(common.mutations, function(x){sd(mut.BEAT[,x])}) > 0 &
sapply(common.mutations, function(x){sd(mut.TARGET[,x])}) > 0

# 14 mutations are common and they are observed at least once
common.mutations.filtered <- names(common.mutations.filtered[common.mutations.filtered])

pdf("output/RF_mutation_predictions_from_cell_composition.pdf")
lapply(common.mutations.filtered, function(x){
  
  dat <- data.frame(y = as.factor(ifelse(mut.all[,x] == 1, "Mutated", "Normal")), deconv.all)
  ...
  fit <- randomForest(as.factor(y)~.,data = train, ntree = 500, na.action = na.exclude, importance = F)
  
  
  cm.train <- as.data.frame(confusionMatrix(predict(fit), as.factor(train$y))$table)
  cm.train$Prediction <- factor(cm.train$Prediction, levels=rev(levels(cm$Prediction)))

  
  gg.train <- 
    ggplot(cm.train, aes(Reference,Prediction, fill= Freq)) +
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction") + theme.gg + ggtitle("Train") + theme(legend.position = "none") 

  
  df.importance <- data.frame(Features = rownames(round(importance(fit), 2)), Importance = round(importance(fit), 2))
  gg.importance <- ggplot(df.importance, aes(Features, MeanDecreaseGini)) + geom_bar(stat = "identity") + 
    ggtitle("Importance") + theme.gg + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    theme(aspect.ratio = 1) 
    
  cm.test <- as.data.frame(confusionMatrix(predict(fit, newdata= test), as.factor(test$y))$table)
  cm.test$Prediction <- factor(cm.test$Prediction, levels=rev(levels(cm$Prediction)))

  
  gg.test <- 
    ggplot(cm.test, aes(Reference,Prediction, fill= Freq)) +
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Reference",y = "Prediction") + theme.gg + ggtitle("Test") + theme(legend.position = "none") 
  
  
  (gg.train | gg.importance | gg.test ) + patchwork::plot_annotation(title = x)
})
dev.off()
...
###GLM 
pdf("output/GLM_mutation_predictions_from_cell_composition.pdf")
lapply(common.mutations.filtered, function(x){
  
  dat <- data.frame(y = mut.all[,x], deconv.all)
  ...
  message(x, "\n")
  gg.train <- ggplot(data.frame(Mutation=as.factor(train$y),Yhat = predict(fit)), aes(Mutation, Yhat, group = Mutation)) + 
    geom_boxplot() + theme.gg + ggtitle("Train") + ylim(c(-10,10)) + 
    stat_compare_means(method = "wilcox.test", label = "p.format", label.y = 9)
  
  df.importance <- data.frame(Features = names(summary(fit)$coefficients[-1,4]), logp = -log10(summary(fit)$coefficients[-1,4]))
  
  gg.importance <- ggplot(df.importance, aes(Features, logp)) + geom_bar(stat = "identity") + 
    ggtitle("Significance") + theme.gg + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    theme(aspect.ratio = 1) + geom_hline(yintercept = -log10(0.05), linetype="dotted")
  
  gg.test <- ggplot(data.frame(Mutation=as.factor(test$y),Yhat = predict(fit, newdata = test[-y])), 
                    aes(Mutation, Yhat, group = Mutation)) + 
    geom_boxplot() + theme.gg + ggtitle("Test") + ylim(c(-10,10)) + 
    stat_compare_means(method = "wilcox.test", label = "p.format", label.y = 9)
...
cell.types <- colnames(deconv.LUMC)

## set the seed to make your partition reproducible
set.seed(123)


pdf("output/RF_cell_composition_predictions.pdf")
lapply(cell.types, function(x){
  dat <-data.frame( y = deconv.all[,x], mut.all[,common.mutations.filtered])#, Blast = ann.row.all$Blast)
  ...
  fit <- randomForest(y~.,data = train, ntree = 500, na.action = na.exclude, importance = F)

  
  gg.train <- ggplot(data.frame(Y = train$y, Yhat = predict(fit)), aes(Y, Yhat)) + geom_point() + theme.gg + 
    theme(aspect.ratio = 1) + ggtitle("Training") + xlab("Deconvoluted Cell Type Ratio") + ylab("Predicted Cell Type Ratio") + 
    labs(subtitle =  paste0("R2: ", round(fit$rsq, 2))) + xlim(c(0,1)) + ylim(c(0,1))
  
  
  df.importance <- data.frame(Features = rownames(round(importance(fit), 2)), Importance = round(importance(fit), 2))
  gg.importance <- ggplot(df.importance, aes(Features, IncNodePurity)) + geom_bar(stat = "identity") + 
    ggtitle("Importance") + theme.gg + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    theme(aspect.ratio = 1) 
  
  
  gg.test  <- ggplot(data.frame(Y = test$y, Yhat = predict(fit, test[,-y])), aes(Y, Yhat)) + geom_point() + theme.gg + 
    theme(aspect.ratio = 1) + ggtitle("Testing") + xlab("Deconvoluted Cell Type Ratio") + ylab("Predicted Cell Type Ratio") + 
    labs(subtitle = paste0("R2: ", round(R2(predict(fit, test[,-y]), test$y, na.rm = T), 2))) + xlim(c(0,1)) + ylim(c(0,1))
...
pdf("output/GLM_cell_composition_predictions.pdf")
lapply(cell.types, function(x){
  dat <-data.frame( y = deconv.all[,x], mut.all[,common.mutations.filtered])#, Blast = ann.row.all$Blast)
  ...
  gg.train <- ggplot(data.frame(Y = train$y, Yhat = predict(fit)), aes(Y, Yhat)) + geom_point() + theme.gg + theme(aspect.ratio = 1) +
    ggtitle("Training") + xlim(c(0,1)) + ylim(c(0,1)) + 
    labs(subtitle = paste0("R2: ", round(R2(predict(fit, train[,-y]), train$y, na.rm = T), 2)))
  ...
  gg.test  <- ggplot(data.frame(Y = test$y, Yhat = predict(fit, test[,-y])), aes(Y, Yhat)) + geom_point() + theme.gg + 
    theme(aspect.ratio = 1) + ggtitle("Testing") + xlab("Real Value") + ylab("Predicted Value") + 
    labs(subtitle = paste0("R2: ", round(R2(predict(fit, test[,-y]), test$y, na.rm = T), 2))) + xlim(c(0,1)) + ylim(c(0,1))
...
```{r clusters ~ mutations}

dat <-data.frame( y = c(clusters.TCGA, clusters.LUMC, clusters.BEAT, clusters.TARGET), 
                  mut.all[,common.mutations.filtered])#, Blast = ann.row.all$Blast)

dat <- dat %>% dplyr::add_count(y)


dat$y.redefined <- ifelse(dat$n > 20, dat$y, "Others")

dat.sampled <- dat %>% group_by(y.redefined) %>% sample_n(20)
dat.sampled <- dat.sampled[, c("y.redefined", common.mutations.filtered)]

## 80% of the sample size
smp_size <- floor(1 * nrow(dat.sampled))
train_ind <- sample(seq_len(nrow(dat.sampled)), size = smp_size)

train <- dat.sampled[train_ind, ]
test  <- dat.sampled[-train_ind,]

fit <- randomForest(as.factor(y.redefined)~.,data = train, ntree = 1000, na.action = na.exclude, importance = F,)

importance(fit)
 
cm.train <- as.data.frame(confusionMatrix(predict(fit), as.factor(train$y.redefined))$table)


pdf("output/Cluster_predictions_RF_training.pdf")
pheatmap(reshape2::acast(cm.train, Prediction ~ Reference, value.var = "Freq"), 
         cluster_cols = F, cluster_rows = F, display_numbers = T, cellheight = 25, cellwidth = 25, number_format = "%d", border_color = NA, fontsize = 12, legend = F)
dev.off()

preds <- predict(fit, newdata = test[,-1])
cm.test <- as.data.frame(confusionMatrix(preds, factor(test$y, levels = levels(preds)))$table)

pdf("output/Cluster_predictions_RF_testing.pdf")
pheatmap(reshape2::acast(cm.test, Prediction ~ Reference, value.var = "Freq"), 
         cluster_cols = F, cluster_rows = F, display_numbers = T, cellheight = 25, cellwidth = 25, number_format = "%d", border_color = NA, fontsize = 12, legend = F)
dev.off()
